...
class MazeEnv(gym.Env):
    def __init__(
        ...
        **kwargs,
        ...
        self.reward_at_goal = 1.1  # reward given when agent reaches goal position
        ...
        self.reward_collision_objball = -0.15
    ...
    def reset(self, goal = None): 
        ...
        self.was_at_goal_already = False  # newly added for reward function
        # Samples a new goal
        if goal is not None:
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        if self.wrapped_env.MANUAL_COLLISION:
            ...
            self.was_in_collision_already = False #newly added
            if collision is not None:
                self.was_in_collision_already = True #newly added
        ...
        outer_reward = self.rewards(next_obs, collision_count) #newly added
    ...
    def rewards(self, obs, objball_collision):
        ...
        # if agent.is_at_goal:
        if self._task.termination(obs):  # if agent is_at_goal
            if self.was_at_goal_already is False:
                # agents should only receive the goal reward once
                rewards = self.reward_at_goal
                self.was_at_goal_already = True  # reset to "False" in the "reset" function.
        # There was collision:
        else:
            if self.was_in_collision_already is True:
                rewards = self.reward_collision_with_wall


            # if objball_collision is not None:
            #     rewards = self.reward_collision_objball * objball_collision
